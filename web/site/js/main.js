...
// New Slider Degustation
     $( document ).ready(function() {
	    ...
	    $(".degustation-home .content a.next, .degustation-home .content a.prev").click(function() {
            ...
            $active.find('h2 b').css({'left' : '-100%', 'transition' : '1s ease'}); 
            setTimeout(function(){moveElement($next, $active, goto_left);}, 400);

        });

        function moveElement($next, $active, goto_left){
            $active.removeClass('sh').addClass('hd'); 			
            $next.addClass('sh').find('h2 b').css('left', goto_left?'100%':'-100%').animate({left:0}, 0); 
        };
        ...
        /*
            ...
            this.bg.animate({'backgroundPosition': (that.positionx+offset/2)+'% 50%' }, 800, function(){ that.is_moving = false; });*/
